# have to work on this again, TiledMapReader was deprecated and will be removed

class Player < Cocos2D::CocosNode
  def initialize
    @sp = Cocos2D::Sprite.new(File.join($RESOURCE_PATH,"ball.png")) # 32x32
  ...
  def initialize
    ...
    @map.layers.each { |layer|
      if layer[:name] == 'physics'
        ...
        node = TiledMap.new(@map.properties.merge(:tiles => File.join($RESOURCE_PATH,"tiles.png"), :data => layer[:data]))
        add_child node
      end
    }
    # add the "ball"
