...
class TestCoordinates < MenuScene
  def initialize
    add_child Sprite.new(File.join($RESOURCE_PATH,"tube.png")).tap { |sp|
      sp.position = [240,160]
    } # too lazy to implement a ColorLayer :-P
    add_child Sprite.new(File.join($RESOURCE_PATH, "black_box.png")), :tag => 0
    add_child Sprite.new(File.join($RESOURCE_PATH, "brown_box.png")), :tag => 1
    add_child Sprite.new(File.join($RESOURCE_PATH, "wheel.png")),     :tag => 2
    add_child Sprite.new(File.join($RESOURCE_PATH, "wheel.png")),     :tag => 3
